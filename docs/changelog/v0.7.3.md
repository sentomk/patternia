...
- Whole-subject `$` binding now forwards the subject by reference instead of
  materializing an eager value copy.
- Subpattern binding through `$(...)` now composes subject binding by
  reference with subpattern bindings.
- Structural binding through `$(has<...>())` now binds member fields by
  reference, avoiding extra copies for large payload members.
...
- Behavioral detail: whole-subject and structural binding paths are now
