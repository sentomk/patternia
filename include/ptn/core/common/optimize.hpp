...
namespace ptn::core::common {
  namespace detail {
    ...
    template <typename Inner, typename Pred>
    struct is_guarded_pattern<
        ptn::pat::mod::guarded_pattern<Inner, Pred>>
        : std::true_type {};

    template <typename T>
    constexpr bool is_guarded_pattern_v = is_guarded_pattern<
        std::decay_t<T>>::value;
    ...
    template <typename Lhs, typename Rhs>
    constexpr bool is_equality_comparable_v = is_equality_comparable<
        Lhs,
        Rhs>::value;
    ...
    template <typename T>
    using literal_subject_key_t = typename literal_subject_key<
        T>::type;

    // Detects std::variant subjects so lowering only engages on
    // variants.
    template <typename T>
    struct is_variant_subject : std::false_type {};

    template <typename... Alts>
    struct is_variant_subject<std::variant<Alts...>>
        : std::true_type {};
    ...
    constexpr variant_dispatch_tier
    variant_dispatch_tier_for_alt_count() {
      if constexpr (AltCount
                    <= k_variant_inline_dispatch_alt_threshold) {
        return variant_dispatch_tier::hot_inline;
      }
      else if constexpr (
          AltCount <= k_variant_segmented_dispatch_alt_threshold) {
    ...
    // Compile-time binary dispatcher for variant alternative
    // indexes. Keeps lowering analysis in optimize.hpp while
    // evaluators provide concrete on-hit / on-miss behavior.
    ...
    struct variant_binary_alt_dispatch {
      ...
      dispatch(std::size_t active_index,
               OnHit     &&on_hit,
               OnMiss    &&on_miss) {
        if constexpr (BeginAlt >= EndAlt || BeginAlt >= AltCount) {
          ...
          constexpr std::size_t mid = BeginAlt
                                      + ((EndAlt - BeginAlt) / 2);

          if (active_index < mid) {
            return variant_binary_alt_dispatch<
                BeginAlt,
                mid,
                AltCount>::dispatch(active_index,
                                    std::forward<OnHit>(on_hit),
                                    std::forward<OnMiss>(on_miss));
          }

          return variant_binary_alt_dispatch<mid, EndAlt, AltCount>::
              dispatch(active_index,
                       std::forward<OnHit>(on_hit),
                       std::forward<OnMiss>(on_miss));
        }
      }
    ...
    constexpr bool is_simple_variant_type_is_pattern_v =
        is_simple_variant_type_is_pattern<
    ...
    constexpr bool is_simple_variant_type_alt_pattern_v =
        is_simple_variant_type_alt_pattern<
            std::decay_t<Pattern>>::value;
    ...
    template <typename Pattern>
    constexpr bool
        is_variant_type_is_pattern_v = is_variant_type_is_pattern<
            std::decay_t<Pattern>>::value;
    ...
    template <typename Pattern>
    constexpr bool
        is_variant_type_alt_pattern_v = is_variant_type_alt_pattern<
            std::decay_t<Pattern>>::value;

    // Wildcard can act as default arm in dispatcher fast paths.
    template <typename Pattern>
    constexpr bool is_wildcard_pattern_v = std::is_same_v<
        std::decay_t<Pattern>,
    ...
    // Matches runtime literal_pattern with default equality
    // comparator.
    template <typename Pattern>
    struct is_simple_literal_pattern : std::false_type {};

    template <typename V>
    struct is_simple_literal_pattern<
        ptn::pat::detail::literal_pattern<V, std::equal_to<>>>
        : std::true_type {};
    ...
    template <auto V>
    struct is_static_literal_pattern<
    ...
    constexpr bool is_simple_literal_dispatch_pattern_v =
        is_simple_literal_pattern_v<Pattern>
        || is_wildcard_pattern_v<Pattern>;
    ...
    struct is_simple_literal_subject_compatible<Pattern,
                                                Subject,
                                                true>
    ...
    template <typename Pattern, typename Subject>
    struct is_static_literal_subject_compatible<Pattern,
                                                Subject,
                                                true>
        : std::bool_constant<
    ...
    struct is_variant_direct_ref_bind_pattern<
        ptn::pat::detail::
            type_is_pattern<Alt, ptn::pat::detail::binding_pattern>>
        : std::true_type {};
    ...
    struct is_variant_direct_ref_bind_pattern<
        ptn::pat::detail::
            type_alt_pattern<I, ptn::pat::detail::binding_pattern>>
        : std::true_type {};

    template <typename Pattern>
    constexpr bool is_variant_direct_ref_bind_pattern_v =
        is_variant_direct_ref_bind_pattern<
            std::decay_t<Pattern>>::value;
    ...
    // Compile-time matcher for simple variant patterns by active
    // alternative.
    template <typename Pattern,
              typename Subject,
    ...
    constexpr bool variant_pattern_maybe_matches_alt_index() {
      ...
      if constexpr (is_wildcard_pattern_v<pattern_t>) {
        ...
        using inner_t = typename guarded_inner_pattern<
            pattern_t>::type;
        return variant_pattern_maybe_matches_alt_index<
            inner_t,
            Subject,
            ActiveIndex>();
      }
    ...
    struct variant_simple_case_index_for_alt {
      ...
      static constexpr std::size_t
          value = simple_variant_pattern_matches_alt_index<
                      pattern_t,
                      Subject,
                      ActiveIndex>()
                      ? I
                      : variant_simple_case_index_for_alt<
                            Subject,
                            CasesTuple,
                            ActiveIndex,
                            I + 1,
    ...
    template <typename Subject,
              ...
              typename CaseIndex,
              std::size_t... AltIndex>
    constexpr auto make_variant_simple_case_index_table(
        std::index_sequence<AltIndex...>) {
      return std::array<CaseIndex,
                        sizeof...(AltIndex)>{static_cast<CaseIndex>(
    ...
    using variant_case_index_t = std::conditional_t<
        ...
        std::conditional_t<
            ...
            std::uint16_t,
            std::conditional_t<(CaseCount <= std::numeric_limits<
                                    std::uint32_t>::max()),
                               std::uint32_t,
                               std::size_t>>>;
    ...
    template <
        typename Subject,
        typename CasesTuple,
        typename SubjectValue = std::remove_cv_t<
            std::remove_reference_t<Subject>>,
    ...
    struct variant_simple_dispatch_metadata {
      ...
      static_assert(case_count <= static_cast<std::size_t>(
                        std::numeric_limits<case_index_t>::max()),
                    "variant simple dispatch case index exceeds "
                    "compact type range");
    ...
    struct variant_typed_case_first_match_index_for_alt {
      ...
      static constexpr std::size_t
          value = variant_pattern_maybe_matches_alt_index<
                      pattern_t,
                      Subject,
                      ActiveIndex>()
                      ? I
                      : variant_typed_case_first_match_index_for_alt<
                            Subject,
                            CasesTuple,
                            ActiveIndex,
                            I + 1,
    ...
    struct variant_typed_case_last_match_index_exclusive_for_alt {
      ...
      static constexpr std::size_t tail =
          variant_typed_case_last_match_index_exclusive_for_alt<
              Subject,
              CasesTuple,
              ActiveIndex,
              I + 1,
    ...
    template <typename Subject,
              typename CasesTuple,
              std::size_t ActiveIndex,
              std::size_t CaseCount = std::tuple_size_v<
                  std::remove_reference_t<CasesTuple>>>
    struct variant_typed_case_range_for_alt {
      static constexpr std::size_t
          begin = variant_typed_case_first_match_index_for_alt<
              Subject,
              CasesTuple,
              ActiveIndex>::value;
      ...
      static constexpr bool has_any = begin < end
                                      && begin < CaseCount;
    };

    // Stores the residual case slice for one variant alternative.
    // A bucketed variant plan first routes by `variant.index()`,
    // then replays only this `[begin, end)` slice to preserve
    ...
    constexpr auto make_typed_variant_case_bucket_table(
      ...
      // The dispatch plan reuses this table instead of recomputing
      // the same range traits at each dispatch site.
      return std::array<bucket_t, sizeof...(AltIndex)>{bucket_t{
    ...
    using variant_compact_alt_index_t = std::conditional_t<
        ...
        std::uint8_t,
        std::conditional_t<(UsedAltCount <= std::numeric_limits<
                                std::uint16_t>::max()),
                           std::uint16_t,
                           std::uint32_t>>;
    ...
    constexpr std::size_t count_typed_variant_compact_dispatch_alts(
        ...
        std::index_sequence<AltIndex...>) {
      // Counts only alternatives that keep a non-empty residual
      // slice. Empty alternatives fall straight to the default path.
      return (std::size_t{0} + ...
    ...
    constexpr auto make_typed_variant_compact_alt_index_map(
      ...
      constexpr CompactIndex
          invalid = std::numeric_limits<CompactIndex>::max();
      ...
      CompactIndex next = 0;
      // Assigns dense cold-path slots only to alternatives that
      // still own a residual slice after planning.
    ...
    struct typed_variant_dispatch_metadata {
      ...
      // Maps each full variant index to the residual case slice that
      // must be replayed after that alternative wins the primary
      // dispatch.
      static constexpr auto
          case_bucket_table = make_typed_variant_case_bucket_table<
              Subject,
              CasesTuple,
              case_count>(std::make_index_sequence<AltCount>{});

      static constexpr std::size_t
          used_alt_count = count_typed_variant_compact_dispatch_alts(
              case_bucket_table,
              std::make_index_sequence<AltCount>{});

      using compact_index_t = variant_compact_alt_index_t<
          used_alt_count>;
      static constexpr compact_index_t
          k_invalid_compact_index = std::numeric_limits<
              compact_index_t>::max();

      // Maps a full variant index to the compact cold-path
      // trampoline table. Alternatives with no residual slice keep
    ...
    struct is_simple_literal_dispatch_case {
      ...
      static constexpr bool
          value = (is_wildcard_pattern_v<pattern_t>
                   || literal_subject_compatible)
                  && std::tuple_size_v<bound_args_t> == 0
                  && std::is_invocable_v<handler_t>;
    };
    ...
    // Describes the work that still remains after primary-key
    ...
    template <typename Pattern>
    using unguarded_pattern_t = typename unguarded_pattern<
        Pattern>::type;

    // Analyzes one case into lowering-friendly IR.
    // This is the seam between the pattern DSL and the planner: new
    // pattern kinds should first describe their discriminator,
    // residual work, and binding shape here before any concrete
    // dispatch plan is chosen.
    template <typename Case, typename Subject>
    struct case_analysis {
      using case_t    = std::remove_reference_t<Case>;
      using pattern_t = traits::case_pattern_t<case_t>;
      // Strips a guard wrapper so discriminator analysis sees the
      // underlying pattern shape rather than the guard adapter
      // itself.
      ...
      // Guards never block key extraction by themselves, but they
      // always force residual work after the primary dispatch key
      // hits.
      static constexpr bool
          is_guarded = is_guarded_pattern_v<pattern_t>;
      // Any non-empty binding tuple means this case changes handler
      // inputs.
      static constexpr bool
          has_bindings = std::tuple_size_v<bound_args_t> != 0;
      static constexpr bool
          is_wildcard = is_wildcard_pattern_v<pattern_t>;

      // Selects the primary key this case exposes to the planner.
      // The order matters: wildcard takes precedence over any inner
      // shape, then static literals, then runtime literals, then
      // variant alt keys. Anything else is opaque to the current
      // planner.
          ...
          is_wildcard
              ...
              : (is_static_literal_pattern_v<core_pattern_t>
                     ...
                     : (is_simple_literal_pattern_v<core_pattern_t>
                            ? case_discriminator_kind::
                                  runtime_literal
                            : ((is_variant_type_is_pattern_v<
                                    core_pattern_t>
                                || is_variant_type_alt_pattern_v<
                                    core_pattern_t>)
                                   ? case_discriminator_kind::
                                         variant_alt
                                   : case_discriminator_kind::
      ...
      // `direct_ref` is the only binding form the current variant
      // lowering can keep without replaying the full matcher.
      static constexpr case_binding_kind
          binding_kind = !has_bindings
      ...
      // - A non-simple variant case still has a usable variant-alt
      // key, but
      //   must replay structural matching inside that alt bucket.
      // - Opaque discriminators and general bindings force
      // structural replay.
      // - Everything else is fully handled by primary dispatch
      // alone.
      static constexpr case_residual_kind residual_kind =
          is_guarded
              ...
              : ((discriminator_kind
                  ...
                  && !is_simple_variant_dispatch_case<
                      case_t,
                      Subject>::value)
                         || discriminator_kind
      ...
      // These flags answer a narrower question than
      // `discriminator_kind`: they say whether the whole case
      // satisfies one concrete plan family.
    ...
    struct is_static_literal_dispatch_sequence {
      ...
      // Every case in the sequence must satisfy the static-literal
      // case rules.
      static constexpr bool
          current_case_ok = is_static_literal_dispatch_case<
              case_t,
              Subject>::value;

      // A wildcard is accepted only as the final default case.
      static constexpr bool
          value = current_case_ok
                  && (is_wildcard_pattern_v<pattern_t>
                          ? (I + 1 == N)
                          : is_static_literal_dispatch_sequence<
                                Subject,
                                CasesTuple,
                                I + 1,
                                N>::value);
    ...
    template <typename Subject, typename... Cases>
    struct is_static_literal_dispatch_cases_tuple<
        Subject,
        std::tuple<Cases...>>
    ...
    struct static_literal_case_count {
      ...
      // Counts only actual static-literal cases. Wildcards
      // contribute to the default slot, but not to the dense literal
      // table density.
      static constexpr std::size_t
          value = (is_static_literal_pattern_v<pattern_t>
                       ? std::size_t{1}
                       : std::size_t{0})
                  + static_literal_case_count<Subject,
                                              CasesTuple,
                                              I + 1,
                                              N>::value;
    };
    ...
    struct static_literal_value_range_case<Subject,
      ...
      // Folds the current static literal into the tail min/max
      // summary.
      static constexpr key_t
          current_value = static_literal_case_value<pattern_t,
                                                    Subject>::value;
      static constexpr bool has_any = true;

      static constexpr key_t min = tail_t::has_any
                                       ? (current_value < tail_t::min
                                              ? current_value
                                              : tail_t::min)
                                       : current_value;

      static constexpr key_t max = tail_t::has_any
                                       ? (current_value > tail_t::max
                                              ? current_value
                                              : tail_t::max)
                                       : current_value;
    };
    ...
    struct static_literal_value_range
        : static_literal_value_range_case<
              ...
              I,
              N,
              is_static_literal_pattern_v<
                  traits::case_pattern_t<std::tuple_element_t<
                      I,
                      std::remove_reference_t<CasesTuple>>>>> {};
    ...
    struct tuple_last_case_is_wildcard
        : std::bool_constant<
              is_wildcard_pattern_v<traits::case_pattern_t<
    ...
    struct static_literal_dispatch_metadata {
      ...
      // `case_count` includes the trailing wildcard default if
      // present.
      static constexpr std::size_t
          case_count = std::tuple_size_v<tuple_t>;
      // `literal_case_count` counts only keyed literal entries.
      static constexpr std::size_t
          literal_case_count = static_literal_case_count<
              Subject,
              CasesTuple>::value;
      using case_index_t = variant_case_index_t<case_count>;

      using value_range_t = static_literal_value_range<Subject,
                                                       CasesTuple>;

      // These values describe the dense candidate key span.
      static constexpr bool
          has_literal_cases            = value_range_t::has_any;
      static constexpr key_t min_value = value_range_t::min;
      static constexpr key_t max_value = value_range_t::max;
      static constexpr std::size_t
          range_size = has_literal_cases
                           ? static_literal_range_width(min_value,
                                                        max_value)
                           : 0u;

      static constexpr case_index_t k_invalid_case_index = std::
          numeric_limits<case_index_t>::max();

      // The final wildcard, when present, is reused as the dense
      // dispatch default slot.
      static constexpr bool
          has_default_case = tuple_last_case_is_wildcard<
              tuple_t,
              case_count>::value;

      static constexpr case_index_t
          default_case_index = has_default_case
                                   ? static_cast<case_index_t>(
                                         case_count - 1)
                                   : k_invalid_case_index;

      // Dense dispatch is worthwhile only when the key span stays
      // small enough and dense enough relative to the number of
      // literal cases.
      static constexpr bool use_dense_table =
          has_literal_cases && literal_case_count > 0
          && case_count <= k_static_literal_dense_dispatch_max_cases
                     &&range_size
                 <= k_static_literal_dense_dispatch_max_span
                     &&range_size
                 <= literal_case_count
                     *k_static_literal_dense_dispatch_max_density;

      // Runtime dense dispatch relaxes the density heuristic for
      // cases where the span is still manageable but the static
      // density check would reject a worthwhile O(1) table.
    ...
    struct is_static_literal_dense_dispatch_enabled
        : std::false_type {};

    template <typename Subject, typename CasesTuple>
    struct is_static_literal_dense_dispatch_enabled<Subject,
                                                    CasesTuple,
                                                    true>
        : std::bool_constant<static_literal_dispatch_metadata<
              Subject,
              CasesTuple>::use_dense_table> {};

    template <typename Subject, typename CasesTuple, bool Eligible>
    struct is_runtime_literal_dense_dispatch_enabled
        : std::false_type {};
    ...
    struct is_runtime_literal_dense_dispatch_enabled<Subject,
        ...
        : std::bool_constant<static_literal_dispatch_metadata<
              Subject,
              CasesTuple>::use_runtime_dense_table> {};
    ...
    struct case_sequence_ir<Subject, std::tuple<Cases...>> {
      using subject_t = std::remove_cv_t<
          std::remove_reference_t<Subject>>;
      // Retains per-case IR so future planners can inspect the whole
      // sequence without re-running case-level trait analysis.
      using analyses_t = std::tuple<
          case_analysis<Cases, Subject>...>;

      static constexpr std::size_t case_count = sizeof...(Cases);

      // `true` if any case still needs work after primary-key
      // routing.
      static constexpr bool
          has_residual_cases = ((case_analysis<Cases, Subject>::
                                     residual_kind
                                 != case_residual_kind::none)
                                || ... || false);

      // `true` if any case binds in a way that blocks the most
      // aggressive lowering shapes.
      static constexpr bool
          has_binding_constraints = ((case_analysis<Cases, Subject>::
                                          binding_kind
                                      != case_binding_kind::none)
                                     || ... || false);

      // `true` if at least one case exposes no planner-visible
      // discriminator.
      ...
      // Rule 1: every case participates in dense static-literal
      // dispatch. This is stronger than "all cases are static
      // literals": the resulting literal span must also satisfy the
      // dense-table heuristics.
      static constexpr bool can_use_static_literal_dispatch =
          is_static_literal_dense_dispatch_enabled<
              subject_t,
              std::tuple<Cases...>,
              ((std::is_integral_v<subject_t>
                || std::is_enum_v<subject_t>)
               && (case_analysis<Cases, Subject>::
                       supports_static_literal_dispatch
                   && ...))>::value;

      // Rule 1: literal cases that satisfy the relaxed density
      // heuristic can use O(1) array-based dispatch even when the
      // stricter static dense check fails.
      ...
      // Rule 1: every case participates in direct runtime literal
      // dispatch. This is the older linear literal lowering. It
      // stays available when dense static-literal dispatch is not
      // legal or not profitable.
      static constexpr bool can_use_simple_literal_dispatch =
          (std::is_integral_v<subject_t>
           || std::is_enum_v<subject_t>)
          && (case_analysis<Cases, Subject>::
                  supports_simple_literal_dispatch
              && ...);

      // Rule 1: every case can dispatch directly from the active
      // variant alt. No residual replay is needed once the active
      // alt is known.
      static constexpr bool can_use_simple_variant_dispatch =
          is_variant_subject_v<subject_t>
          && (case_analysis<Cases, Subject>::
                  supports_variant_direct_dispatch
              && ...);

      // Rule 2: the variant alt is still a usable primary key even
      // if some cases need residual replay within the selected
      // bucket. Today this is the only bucketed lowering family. The
      // planner will still reject non-variant subjects here.
      static constexpr bool
          can_use_variant_alt_dispatch = is_variant_subject_v<
              subject_t>;

      // Collapses the aggregate IR back to the three lowering rules.
      // Precedence matters:
      // - `full` wins whenever a direct literal or direct variant
      // plan exists.
    ...
    // This stays declarative so future planners can reuse the same
    // legality checks without depending on evaluator internals.
    template <typename Subject, typename CasesTuple>
    struct lowering_analysis {
      using ir_t = case_sequence_ir<
          Subject,
          std::remove_reference_t<CasesTuple>>;

      // Re-exports the sequence IR in the shape expected by plan
      // selection.
      static constexpr bool can_use_static_literal_dispatch = ir_t::
          can_use_static_literal_dispatch;
      static constexpr bool
          can_use_runtime_literal_dense_dispatch = ir_t::
              can_use_runtime_literal_dense_dispatch;
      static constexpr bool can_use_simple_literal_dispatch = ir_t::
          can_use_simple_literal_dispatch;
      static constexpr bool can_use_simple_variant_dispatch = ir_t::
          can_use_simple_variant_dispatch;
      static constexpr bool can_use_variant_alt_dispatch = ir_t::
          can_use_variant_alt_dispatch;
    ...
    struct sequential_dispatch_plan {
      ...
      static constexpr dispatch_plan_kind
          kind = dispatch_plan_kind::sequential;
      // Rule 3: no keyed lowering is legal, so evaluator replays all
      // cases.
      static constexpr lowering_legality
          legality = lowering_legality::none;
    };
    ...
    struct literal_linear_dispatch_plan {
      ...
      static constexpr dispatch_plan_kind
          kind = dispatch_plan_kind::literal_linear;
      // Rule 1: each case is already a direct literal dispatch case.
      static constexpr lowering_legality
          legality = lowering_legality::full;
    };
    ...
    struct static_literal_dense_dispatch_plan {
      ...
      static constexpr dispatch_plan_kind
          kind = dispatch_plan_kind::static_literal_dense;
      static constexpr lowering_legality
          legality = lowering_legality::full;

      // Re-exports the dense-table metadata the evaluator needs at
      // runtime.
      static constexpr std::size_t
          case_count = metadata_t::case_count;
      static constexpr std::size_t
          literal_case_count = metadata_t::literal_case_count;
      static constexpr std::size_t
          range_size = metadata_t::range_size;
      static constexpr bool
          has_default_case = metadata_t::has_default_case;
      static constexpr key_t min_value = metadata_t::min_value;
      static constexpr key_t max_value = metadata_t::max_value;
      static constexpr case_index_t
          default_case_index = metadata_t::default_case_index;
      static constexpr case_index_t
          k_invalid_case_index = metadata_t::k_invalid_case_index;
    };
    ...
    struct variant_simple_dispatch_plan {
      using subject_t     = SubjectValue;
      using cases_tuple_t = std::remove_reference_t<CasesTuple>;
      using metadata_t    = variant_simple_dispatch_metadata<
             Subject,
             CasesTuple,
             SubjectValue,
             AltCount>;
      ...
      static constexpr dispatch_plan_kind
          kind = dispatch_plan_kind::variant_simple;
      static constexpr lowering_legality
          legality = lowering_legality::full;

      // All variant alternatives stay addressable in the simple
      // plan.
      static constexpr std::size_t alt_count = AltCount;
      static constexpr std::size_t
          case_count = metadata_t::case_count;
      static constexpr std::size_t used_alt_count = AltCount;
      static constexpr variant_dispatch_tier
          tier = variant_dispatch_tier_for_alt_count<AltCount>();
      // Maps each alternative directly to the terminal case chosen
      // for it.
      static constexpr auto
          case_index_table = metadata_t::case_index_table;
      // The simple plan keeps every alternative, so the compact map
      // is the identity function.
      static constexpr auto compact_alt_index_map =
          make_identity_variant_compact_alt_index_map<
              compact_index_t,
              AltCount>(std::make_index_sequence<AltCount>{});
      static constexpr compact_index_t
          k_invalid_compact_index = std::numeric_limits<
              compact_index_t>::max();
      ...
      struct case_entry {
        static_assert(AltIndex < AltCount,
                      "variant alt index out of range");

        // `case_index` names the terminal case that handles this
        // alternative.
        static constexpr case_index_t
            case_index = case_index_table[AltIndex];
        // `false` means this alternative falls through to
        // `otherwise`.
    ...
    struct variant_alt_bucketed_dispatch_plan {
      using subject_t     = SubjectValue;
      using cases_tuple_t = std::remove_reference_t<CasesTuple>;
      using metadata_t    = typed_variant_dispatch_metadata<
             Subject,
             CasesTuple,
             SubjectValue,
             AltCount>;
      ...
      static constexpr dispatch_plan_kind
          kind = dispatch_plan_kind::variant_alt_bucketed;
      static constexpr lowering_legality
          legality = lowering_legality::bucketed;

      // Only alternatives with a non-empty residual slice stay in
      // the compact cold-path table.
      static constexpr std::size_t alt_count = AltCount;
      static constexpr std::size_t
          case_count = metadata_t::case_count;
      static constexpr std::size_t
          used_alt_count = metadata_t::used_alt_count;
      static constexpr variant_dispatch_tier
          tier = variant_dispatch_tier_for_alt_count<AltCount>();
      // The evaluator uses this map only on the cold path.
      static constexpr auto
          compact_alt_index_map = metadata_t::compact_alt_index_map;
      // This table is the real payload of bucketed lowering: each
      // alternative maps to the `[begin, end)` slice that must be
      // replayed.
      static constexpr auto
          case_bucket_table = metadata_t::case_bucket_table;
      static constexpr compact_index_t
          k_invalid_compact_index = metadata_t::
              k_invalid_compact_index;
      ...
      struct case_bucket {
        ...
        // Type-level view for a single precomputed residual case
        // bucket. The evaluator replays exactly this half-open
        // interval on a hit.
        static constexpr std::size_t
            begin = case_bucket_table[AltIndex].begin;
        static constexpr std::size_t
                              end = case_bucket_table[AltIndex].end;
        static constexpr bool has_any = case_bucket_table[AltIndex]
                                            .has_any;
      };
    };
    ...
    struct literal_runtime_dense_dispatch_plan {
      ...
      static constexpr dispatch_plan_kind
          kind = dispatch_plan_kind::literal_runtime_dense;
      static constexpr lowering_legality
          legality = lowering_legality::full;
    ...
    struct dispatch_plan_for_kind<
      ...
      using type = static_literal_dense_dispatch_plan<Subject,
                                                      CasesTuple>;
    };
    ...
    struct dispatch_plan_for_kind<
      ...
      using type = literal_runtime_dense_dispatch_plan<Subject,
                                                       CasesTuple>;
    };
    ...
    struct dispatch_plan_for_kind<
      ...
      using type = variant_alt_bucketed_dispatch_plan<Subject,
                                                      CasesTuple>;
    };
    ...
    template <typename Subject, typename CasesTuple>
    using dispatch_plan = typename dispatch_plan_selector<
        Subject,
        CasesTuple>::type;
