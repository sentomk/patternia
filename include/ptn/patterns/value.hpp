...
namespace ptn::patterns {
  ...
  using value_store_t = std::conditional_t<std::is_array_v<std::remove_reference_t<V>> ||
  ...
  /* factory: automatic selection of the storage type based on the entry parameter */
  template <typename V>
  ...
  constexpr auto ci_value(V &&v) {
    using store_t = value_store_t<V>;
    return value_pattern<store_t, iequal_ascii>{store_t(std::forward<V>(v)), iequal_ascii{}};
  }
